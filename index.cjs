...
const { Client, GatewayIntentBits, Events, SlashCommandBuilder, REST, Routes, PermissionsBitField } = require('discord.js');
...
const commands = [
  ...
  new SlashCommandBuilder().setName('炸私聊').setDescription('在私聊中發送炸訊息').toJSON()
];
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === '炸私聊') {
        ...
        await interaction.reply({ content: '🚀 私聊炸訊息開始發送！', ephemeral: true });
        const parts = splitMessage(spamMessages['炸1']);
        for (let i = 0; i < 5; i++) {
          for (const p of parts) await interaction.user.send(p).catch(() => null);
          await sleep(500);
        }
        return;
      }
      ...
      if (Object.keys(spamMessages).includes(cmd)) {
        ...
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`send-${cmd}`).setLabel('🚀 發送炸訊息').setStyle(ButtonStyle.Danger)
        );

        await interaction.reply({ content: `按下按鈕開始發送 **${cmd}**`, components: [row], ephemeral: true });
        return;
      }
    }
    ...
    if (interaction.isButton()) {
      ...
      if (userId !== CREATOR_ID)
        return interaction.reply({ content: '❌ 你沒有權限點擊這個按鈕', ephemeral: true });
      ...
      await interaction.reply({ content: `💥 開始發送 **${cmd}** 到所有可見頻道！`, ephemeral: true });
      const parts = splitMessage(spamMessages[cmd]);
      const guild = interaction.guild;
      if (!guild) return;

      const targetChannels = guild.channels.cache.filter(c => c.isTextBased() && c.viewable);
